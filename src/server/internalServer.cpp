...
SearchInfo::SearchInfo(const std::string& pattern, GeoQuery geo_query)
  : pattern(pattern),
    has_geo_query(true),
    geo_query(geo_query)
{}
...
{
  try {
    pattern = request.get_argument("pattern");
  ...
  try {
    ...
    geo_query = GeoQuery(latitude, longitude, distance);
    has_geo_query = true;
  } catch(const std::out_of_range&) {}
    catch(const std::invalid_argument&) {}

  if (!has_geo_query && pattern.empty()) {
