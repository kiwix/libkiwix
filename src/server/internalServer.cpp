...
std::string InternalServer::getNoJSDownloadPageHTML(const std::string& bookId, const std::string& userLang) const
{
  ...
  // distinct metalink4+xml link; they tag the .meta4 URL as AcquisitionLinkKind::DIRECT
  // instead, so fall back to that when there's no dedicated metalink link.
  auto metalinkUrl = book.getUrl(kiwix::Book::AcquisitionLinkKind::META4);
  auto bookUrl = kiwix::stripSuffix(
    metalinkUrl.empty() ? book.getUrl(kiwix::Book::AcquisitionLinkKind::DIRECT) : metalinkUrl,
    ".meta4");
