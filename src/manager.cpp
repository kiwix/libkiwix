...
{
  return addBookFromPathAndGetId(pathToOpen, pathToSave,
                                  BookAcquisitionUrls{url, "", ""},
                                  checkMetaData);
}
  ...
  if (this->readBookFromPath(pathToOpen, &book)) {
    ...
    if (!checkMetaData
        || (!book.getTitle().empty() && !book.getLanguages().empty()
            && !book.getDate().empty())) {
      if (!urls.zim.empty()) {
        book.setUrl(Book::ACQUISITION_MIMETYPE_ZIM, urls.zim);
      }
      if (!urls.meta4.empty()) {
        book.setUrl(Book::ACQUISITION_MIMETYPE_ZIM_METALINK, urls.meta4);
      }
      if (!urls.torrent.empty()) {
        book.setUrl(Book::ACQUISITION_MIMETYPE_BITTORRENT, urls.torrent);
      }
      manipulator.addBookToLibrary(book);
      return book.getId();
    }
