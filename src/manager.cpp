...
{
  const bool looksLikeOpds =
      kiwix::getFileContent(libraryPath).find("<feed") != std::string::npos;
  return looksLikeOpds ? FileFormat::OPDS : FileFormat::XML;
}
  ...
  bool trustLibrary)
{
  ...
  if (result) {
    if (detectFormat(path) == FileFormat::OPDS) {
      // The URL host is not known/relevant when reading a file from disk.
