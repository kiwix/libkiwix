...
TEST(ManagerTest, addBookFromPathAndGetIdWithAcquisitionUrlsTest)
{
    ...
    ASSERT_NE(bookId, "");
    kiwix::Book book = lib->getBookById(bookId);
    EXPECT_EQ(book.getUrl(kiwix::Book::ACQUISITION_MIMETYPE_ZIM), urls.zim);
    EXPECT_EQ(book.getUrl(kiwix::Book::ACQUISITION_MIMETYPE_ZIM_METALINK), urls.meta4);
    EXPECT_EQ(book.getUrl(kiwix::Book::ACQUISITION_MIMETYPE_BITTORRENT), urls.torrent);
